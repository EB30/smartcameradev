...
# There is a high chance the directory has been relocated. We are not using GstRtspServer
...
#These don't have to be used yet____
...
def make_elm_or_print_err(factoryname, name, printedname):
    #This is a good function that can help debug when creating elements on the pipeline
...
#Above should be a Facemask class and related helper functions

 #input_filename derived from sys.argv[1]
def main( 
    config: dict,
    input_filename: str,
    ...
    global frame_number #number of frames
    global start_time # time to start stream
    global end_time # time to end stream
    global e_interrupt #For the control time

#config is a dictionary of dicitonaries with many related configurtaions. 


    udp_ports = {int(config["maskcam"]["udp-port-streaming"])} #This is a dictionary of video port number 5400
    load_udp_ports_filesaving(config, udp_ports) #We take information from config ["maskcam"]["udp-ports-filesave"] whihc is 5401, 5402
    ...
    #apply Gst.ElementFactory.make("some factory","some name" )
    #We need to have a source, streammux (for batching), pgie, nvosd, 

    print("creating pipeline\n")
    pipeline = Gst.Pipeline()

    if not pipeline:
        print("Unable to create pipline object")

    #create plugins with Gst.ElementFactory.make()
    if camera_input:
        if usbcam_input:
            input_device = input_filename[len(USBCAM_PROTOCOL) :]
            source = make_elm_or_print_err("v412src", "v4l2-camera-source", "Camera input") 
            source.set_property("device", input_device)
            nvvidconvsrc = make_elm_or_print_err("nvvideoconvert", "convertor_src2", "Convertor src 2")
    ...
    #For Sink we can try vlc412sink
   #Mysterious converting sequence from https://github.com/NVIDIA-AI-IOT/deepstream_python_apps/blob/master/apps/deepstream-test1-usbcam/deepstream_test_1_usb.py
        ...
        caps_vidconvsrc.set_property("caps", Gst.Caps.from_string("video/x-raw(memory:NVMM)"))
            ...
            #Usually I don't have this logic flow
        pass
        #source_bin = create_source_bin(0, input_filename)
    #Typical: streammux, pgie, nvvidconv, nvosd

    #in this case streammux, pgie, convert_pre_osd, nvosd, queue, convert_post_osd,
    # capsfilter, encoder, codeparser, rtppay, splitter_file_udp, queue_udp, multiudpsink, 
    ...
    #Gst-nvvideoconvert from nv12 to RGBA
    convert_pre_osd = make_elm_or_print_err(
        "nvvideoconvert", "convert_pre_osd", "Converter NV12->RGBA"
    )
    #Takes in RGBA buffer outputs a RGBA buffer
   
    nvosd = make_elm_or_print_err("nvdosd", "onscreendisplay", "OSD (nvosd)")
...
# Encoder: H265 has more efficient compression
    ...
    if output_filename is not None:
        ...
        #Usually doesn't pass 
        # Fake sink, no save
        fakesink = make_elm_or_print_err("fakesink", "fakesink", "Fake Sink")
    
    #COMPLETED CREATING GOBJECTS with Deepstream
    #Adding objects into the pipeline
    
    #Adding elements to the pipeline
    pipeline.add(source)  #v412src for accepting input from v412 devices such as usb webcams
    pipeline.add(caps_camera) #limits the data format
    pipeline.add(vidconvsrc) #video converter ensures a superset of raw formats are supported; convertor_src1
    pipeline.add(nvvidconvsrc) #to convert incoming raw buffers to NVMM (NvBufSurface API); convertor_src2
    #NVMM memory
    pipeline.add(caps_vidconvsrc) #NVMM caps for input stream

    pipeline.add(streammux) #Stream-muxer: nvstreammux plugin for a batch the video; 
    #input NV12/RGBA buffers from an arbitrary number of sources


    pipeline.add(pgie) #Primary GPU Inference Engine by nvinfer plugin

    pipeline.add(convert_pre_osd) #Converting from NV12 to RGBA
    pipeline.add(nvosd) #Accepts only RGBA buffers as input and draws
    pipeline.add(queue) #queue
    pipeline.add(convert_post_osd) #Converting from RGBA to NV12
    pipeline.add(capsfilter) #capsfilter Video capabilities: check format and GPU/CPU location
    pipeline.add(encoder) #nvv4l2h264enc encoder; https://developer.download.nvidia.com/embedded/L4T/r32_Release_v1.0/Docs/Accelerated_GStreamer_User_Guide.pdf?n2jhQGNLK1ZixVKc1o2c8iBoUmRQWX2bQKgm6Sff9B4_IX8prVQkRcuv0lhRJw98JM3U68d4nzfb_7nPdLX7GxkN-xYFZ0uuzxfa-G6y-OLxope-SLu3lqAoBhFJ-7874KolBzY1MxX52JfcUJbyFi8EaXb8ylW_oQiVVsZxmoeJotnbXl4

    pipeline.add(splitter_file_udp)#Splits data into multiple pads for capturing video to display on screen and for saving the file
    #https://gstreamer.freedesktop.org/documentation/coreelements/tee.html?gi-language=c

    #Assume there is no output file
    pipeline.add(queue_file) # The queue will create a new thread on the source pad to decouple the processing on sink and source pad.
    pipeline.add(codeparser) #h264parse
    pipeline.add(container) #qtmux: muxer for Quicktime or .mov files

    #Output UDP video
    pipeline.add(queue_udp) # udp queue
    pipeline.add(rtppay)# Payload-encode H264 video into RTP packets (RFC 3984)
    pipeline.add(multiudpsink) #network sink that sends UDP packets to multiple clients;
    # can be combined with rtp payload encoders to implement RTP streaming.

    #Then we convert the video into rtsp streaming and mp4 video files

    #Linking the pipeline elements
    ...
    tee_file = splitter_file_udp.get_request_pad("src_%u") #For saving file
    tee_udp = splitter_file_udp.get_request_pad("src_%u") #For udp
    ...
    #Then add probe to get informed of meta data generated, we add probe to
    #the sink pad of the osd element, since by that time, the buffer would have all of the metadata
...
if __name__ == "__main__":
    ...
    else: #This is exactly 2 arguments
